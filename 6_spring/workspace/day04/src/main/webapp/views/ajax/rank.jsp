...
<script>
	...
	getData();
	setInterval(getData,20000); //20초마다 한번씩 getData 함수호출(서버와 통신)	
	setInterval(change,2000); //2초마다 한번씩 chanege 함수호출
	...
	function getData(){
		...
		xhr.onreadystatechange = function(){
			if(xhr.readyState == 4){
				if(xhr.status == 200){
					//JSON.parse("JSON 형태의 문자열 데이터") : 안에 적혀있는 문자열로 자바스크립트 객체화

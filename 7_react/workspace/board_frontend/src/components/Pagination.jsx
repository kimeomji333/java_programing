...
const Pagination = ({pageMaker}) => {
    ...
    return (
        <div className="pagination w1000 tac">
            {
                pageMaker.prev ? <a className="btn changePage" href={startPage-1} key={startPage-1} onClick={clickBtn}>&lt;</a> : ""
            }
            {elList}
            {
                pageMaker.next ? <a className="btn changePage" href={endPage
                    +1} key={endPage+1} onClick={clickBtn}>&gt;</a> : ""
            }
        </div>
    )
}
